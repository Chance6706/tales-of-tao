...
## Deep Dive: Economy System (Bannerlord-Inspired)

### Architecture

The economy has 4 layers that process in order during the Income Phase:
1. **PRODUCTION** → Resources generated by tiles, buildings, disciples
2. **INCOME** → Tael/Qi added to stockpile from all sources
3. **UPKEEP** → Tael/Qi consumed by disciples, buildings, supply lines
4. **MARKET** → Price adjustments, trade route income, settlement tribute

### Key Design Principles (from Bannerlord)
1. **Regional price variation** — different areas produce different goods at different prices
2. **Dynamic supply/demand** — prices fluctuate based on what's bought/sold
3. **Trade route arbitrage** — profit from buying low in one place, selling high in another
4. **Passive income** — trade routes and settlements generate income automatically
5. **Risk/reward** — war and bandits disrupt trade but create opportunities
6. **Face/Renown effects** — reputation affects prices and AI behavior

### Tael Income Formula

```
Income = TempleIncome + Σ(TradeRouteIncome) + Σ(SettlementTribute) + Bonuses

Where:
  TempleIncome = TempleTier × 5
  TradeRouteIncome = 10 × (1 + distance/20) × commodityDifferential [capped at 30]
  SettlementTribute = TrustMult × SettlementTier × 5 [Friendly+ only]
  PigYearBonus = TotalTradeRouteIncome × 0.20 [Pig year only]
  NamgungBonus = +10% to Temple + TradeRoute income
```

### Tael Upkeep Formula

```
Upkeep = Σ(DiscipleUpkeep) + Σ(BuildingUpkeep) + SupplyLineSurcharge + LuxuryScaling

Where:
  DiscipleUpkeep: Peon=1, Outer=3, Inner=8, Elder=20, HighElder=50
  SupplyLineSurcharge = max(0, distance - 5) × 0.5 per unit beyond 5 hexes
  LuxuryScaling = +10/turn per 500 surplus above 500 [anti-inflation, cap +50]
```

### Market Pricing

```
BuyPrice = BasePrice × PavilionMarkup × FaceModifier × (1 + 0.10 × qty)
SellPrice = BasePrice × 0.60 × (1 - 0.05 × qty) [saturation beyond 5 units]

Price drift: lerp(current, base, 0.05) per turn

Pavilion markup: T1=×1.20, T2=×1.10, T3=×1.00
Face modifier: Esteemed=×0.90, Respected=×0.95, Neutral=×1.00, Diminished=×1.05, Disgraced=×1.10
```

### Face & Renown

**Face (0-100, starts at 50):** Affects market prices and AI attack hesitation
- Esteemed (≥80): Buy ×0.90, AI needs +20% CP advantage to attack
- Respected (60-79): Buy ×0.95, AI needs +10%
- Neutral (35-59): No modifier
- Diminished (20-34): Buy ×1.05, AI needs -10%
- Disgraced (≤20): Buy ×1.10, AI actively seeks to attack

**Renown (0-∞):** Global reputation, drives Influence victory
- Sources: Combat victories (+8-15), research completion (+5-40), tournaments (+30-50), settlement protection (+10)
- Losses: Breaking pacts (-20), attacking allies (-30), witnessed sacrifices (-15)
- Thresholds: >25% global = AI considers alliances, >50% = Militant AI aggressive, >75% = Influence victory check

### Bankruptcy Handler

When Tael < 0 at end of Income Phase:
- 20% chance per peon to desert
- Build queue paused
- Dissent +10
- Face -10
...
- [ ] Tael income matches GDD formulas for all sources
- [ ] Upkeep calculation correct for all disciple ranks
- [ ] Trade route income scales with distance and commodity differential
- [ ] Market prices drift toward equilibrium (5% per turn)
- [ ] Bulk purchase/sell penalties apply correctly
- [ ] Face modifier affects market prices
- [ ] Trust builds and decays correctly
- [ ] Settlement tribute collected at Friendly+
- [ ] Bankruptcy handler triggers at Tael < 0
- [ ] Anti-inflation mechanics activate at surplus > 500
- [ ] Supply line surcharge applies beyond 5 hexes
- [ ] Pig year bonus applies correctly
- [ ] Sect traits (Namgung, etc.) apply correctly
- [ ] Save/load preserves all economy state

---

## Deep Dive: Settlements & Influence System

### Settlement Types & Placement

| Type | Pop | Provides at Friendly+ | Min Spacing | Count (4K tiles) |
|------|-----|----------------------|-------------|-------------------|
| Village | 50-200 | Peon recruits (2/turn), basic commodities | 8 hex | ~15-20 |
| Town | 200-1000 | Higher trade, diverse commodities, Outer recruits | 12 hex | ~8-12 |
| Trade Post | 50-300 | Premium rates (×1.3), rare commodities (Jade, Tea) | 15 hex | ~4-6 |
| Wandering Tribe | Mobile | One-time Outer mercenary, Horse movement bonus | N/A | ~3-5 |
| Hermit's Dwelling | 1 | Technique scroll OR temporary Elder hire | 20 hex | ~2-3 |

### Trust Tier System

| Tier | Range | Trade Route | Recruitment | Tribute | Influence Vote |
|------|-------|-------------|-------------|---------|----------------|
| Hostile | 0-19 | No | No | No | No |
| Wary | 20-39 | Peng Clan only | No | No | No |
| Neutral | 40-59 | No | No | No | No |
| Friendly | 60-79 | Yes | Peon (2/turn) | Yes | No |
| Devoted | 80-100 | Yes | +1 Outer/turn | Yes (×1.5) | Yes |

### Trust Building Actions

| Action | Trust Gain | Cost | Cooldown |
|--------|-----------|------|----------|
| Send Tael Gift | +5 | 50 Tael | 3 turns |
| Send Commodity Gift | +8 | commodity | 3 turns |
| Complete Defence Request | +15 | 0 | Event-driven |
| Complete Delivery Request | +10 | commodity | Event-driven |
| External Affairs Influence | +5 | 20 Tael | 2 turns |
| Emei Emissary | +10 | 20 Tael | 2 turns |
| Dog Year passive | +15% all gains | 0 | Year 11 only |

### Trust Decay
- -1/turn after 10 turns of no action
- -40 immediately if attacked
- Triggers Bandit Uprising event

### Influence Victory

Requires: ≥60% global Renown AND ≥40% settlements at Devoted

### Settlement AI Behavior
- Hostile settlements: 10% chance/turn to spawn Bandits (strength scales with game turn)
- Friendly+ settlements: Generate requests (Defence, Delivery, Tournament)
- Wandering Tribes: Move 1 tile/turn randomly

### Settlement Testing Checklist

- [ ] Settlements generate with correct spacing
- [ ] Trust builds correctly from all actions
- [ ] Trust decays after 10 turns of no action
- [ ] Trade routes only available at Friendly+
- [ ] Peon recruitment works at Friendly+
- [ ] Tribute collection works at Friendly+
- [ ] Devoted settlements vote for Influence victory
- [ ] Bandit spawning works for Hostile settlements
- [ ] Wandering Tribes move 1 tile/turn
- [ ] Hermit's Dwelling provides one-time reward
- [ ] Settlement requests generate and complete correctly
